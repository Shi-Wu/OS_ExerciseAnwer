...
class fs:
    ...
    # YOUR CODE, 2013011372
        ...
        parentBnum = self.inodes[parentInum].getAddr()
        self.data[parentBnum].delDirEntry(tfile)
    ...
    # YOUR CODE, 2013011372
        ...
        if self.data[parentBnum].getFreeEntries() == 0:
            return -1
        # if the newfile was already in parent dir?
        if self.data[parentBnum].dirEntryExists(newfile):
            return -1
        ...
        self.data[parentBnum].addDirEntry(newfile, inum)
    # DONE
        return inum
    ...
    # YOUR CODE, 2013011372
        ...
        if ftype == 'd':
            bnum = self.dataAlloc()
            self.inodes[inum].setAll(ftype, bnum, 2)
            self.data[bnum].setType(ftype)
            self.data[bnum].addDirEntry('.', inum)
            self.data[bnum].addDirEntry('..', parentInum)
    ...
    def createSoftLink(self, target, newfile, parent):
        ...
        if inum < 0 or bnum < 0:
            return -1
        self.inodes[inum].setAll('s', bnum, 1)
        self.data[bnum].setType('s')
        self.data[bnum].setTarget(target)
        self.inodes[parentInum].incRefCnt()
        self.data[parentBnum].addDirEntry(newfile, inum)
        return inum
    ...
    # YOUR CODE, 2013011372
        if self.inodes[inum].getType() == 'f':
            ...
            if bnum < 0:
                return -1
            # write file data
            self.data[bnum].setType('f')
            self.data[bnum].addData(data)
            self.inodes[inum].setAddr(bnum)
        if self.inodes[inum].getType() == 's':
            bnum = self.inodes[inum].getAddr()
            target = self.data[bnum].target
            if target not in self.nameToInum or self.inodes[self.nameToInum[target]].getType() == 'free':
                print 'SoftLink does not exist'
                return -1
    ...
    def run_softlink_test(self):
        print 'Soft Link Test Begin:\n'
        print 'Initial state'
        print ''
        self.dump()
        print ''
        ...
        print 'softlink("/u", "/v");'
        self.files.append("/v");
        self.nameToInum['/v'] = self.createSoftLink('/u', 'v', '/')
        print ''
        self.dump()
        print ''

        print 'fd=open("/v", O_WRONLY|O_APPEND); write(fd, "a", BLOCKSIZE); close(fd);'
        self.writeFile('/v', "a")
        print ''
        self.dump()
        print ''
        ...
        print 'softlink("/v", "/u");'
        self.files.append("/u");
        self.nameToInum['/u'] = self.createSoftLink('/v', 'u', '/')
        print ''
        self.dump()
        print ''
