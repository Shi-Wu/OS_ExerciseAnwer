...
alloc_proc(void) {
     ...
     */
        ...
        proc->pid = -1;
        proc->cr3 = boot_cr3;
		cprintf("alloc_proc successed!\n");
    }
	else cprintf("alloc_proc failed!\n");
    return proc;
}
...
proc_run(struct proc_struct *proc) {
    if (proc != current) {
        ...
        local_intr_save(intr_flag);
        {
            ...
            lcr3(next->cr3);
			cprintf("switch from proc pid %d to proc pid %d\n",prev->pid,next->pid);
...
repeat:
	cprintf("do_wait: the proc pid %d begin to do_wait!\n", pid);
    haskid = 0;
	list_entry_t *list = &proc_list,  *le = list;
	while ((le = list_next(le)) != list) {
		proc = le2proc(le, list_link);
    ...
    if (haskid) {
		cprintf("do_wait: has kid  proc pid %d begin\n",proc->pid);
...
init_main(void *arg) {
	...
	cprintf("schedule: the proc pid %d is running!\n", current->pid);
