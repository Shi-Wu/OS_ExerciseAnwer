...
class scheduler:
    ...
    def move_to_ready(self, expected, pid=-1):
        #YOUR CODE
        if pid == -1:
            tempId = self.curr_proc
        else:
            tempId = pid
        if self.proc_info[tempId][PROC_STATE] == expected:
            self.proc_info[tempId][PROC_STATE] = STATE_READY
        return
    ...
    def next_proc(self, pid=-1):
        ...
        if pid == -1:
            tempId = (self.curr_proc + 1) % procNum
        else:
            tempId = (pid + 1) % procNum
            
        num_unsearched = len(self.proc_info)
        while self.proc_info[tempId][PROC_STATE] in [STATE_WAIT, STATE_DONE] and self.get_num_runnable() > 0 and num_unsearched > 0:
            tempId = (tempId + 1) % procNum
            num_unsearched -= 1
        self.curr_proc = tempId
