...
3.ucore的进程控制块数据结构是如何组织的？主要字段分别表示什么？有哪些函数对它进行了修改？有哪些函数用到它？试着解释为何要修改或使用它。
```
arch_proc_struct
mm_struct
need_resched
...
---------------------
Ans:
只需在ucore_lab\labcodes_answer\lab8_result\kern\init\目录下的init.c 文件中，proc_init()函数执行后添加:

		cprintf("idleproc's pid: %d, initproc's pid: %d\n", idleproc->pid, initproc->pid);

即可。
在ucore_lab\labcodes_answer\lab8_result\kern\process\目录下的proc.c文件中可以看到关于idleproc和initproc具体的结构。
