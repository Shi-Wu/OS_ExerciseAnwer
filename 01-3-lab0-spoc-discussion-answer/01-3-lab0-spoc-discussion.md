...
# [HARD]分析和实验os1/os3.c，需要完成的内容包括：
 ...
 - os1中的task1和task2的堆栈的起始和终止地址是什么？
	- task0的堆栈的起始地址为 *(204+pc)； 终止地址为 *(308+pc).
	- task1的堆栈的起始地址为 task1_stack+50； 终止地址为 0.

 - os1是如何实现任务切换的？
	- main函数中设置时钟中断，超时时间为5000个时钟周期。中断向量设置alltraps函数，内部包含trap函数，trap函数则是根据current的奇偶性来回切换当前正在执行的task的sp，切换的过程由swtch函数处理。从而完成任务切换。

 - os3中的task1和task2的堆栈的起始和终止地址是什么？
	- task0的堆栈的起始地址为 *(4008+pc)； 终止地址为 *(24+pc).
	- task1的堆栈的起始地址为 task1_stack+1000； 终止地址为 0.

 - os3是如何实现任务切换的？
	- os3同样设置了与os1相同的时钟中断和中断向量，只是切换方式则是分为在用户态下发生系统调用，还是用户态下发生时钟超时。发生系统调用，则输出x；发生超时，则像os1一样将两个task的sp切换完成任务切换。
